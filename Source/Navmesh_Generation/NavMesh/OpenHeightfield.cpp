...
void AOpenHeightfield::FindRegionConnections(UOpenSpan* Span, int NeighborDirection, TArray<int>& RegionConnection)
{
	...
	while (LoopCount < UINT16_MAX)
	{
		...
		else
		{
			...
			Direction = DecreaseNeighborDirection(Direction, 1);
		}
...
void AOpenHeightfield::CleanNullRegionBorders()
{
	...
	for (auto& Span : Spans)
	{
		...
		do
		{
			...
			if (CurrentSpan->RegionID == NULL_REGION)
			{
				...
				EdgeDirection = IncreaseNeighborDirection(EdgeDirection, 2);
			}
			else if (!UseOnlyNullSpans)
			{
			...
			bool EncompassedInNullRegion = ProcessNullRegion(WorkingSpan, EdgeDirection);
			if (EncompassedInNullRegion)
			{
...
void AOpenHeightfield::PartialFloodRegion(UOpenSpan* Span, int BorderDirection, int NewRegionID)
{
	TArray<UOpenSpan*> SpanStack;
	TArray<int> SpanBorderDistance;

	int AntiBorderDirection = IncreaseNeighborDirection(BorderDirection, 2);
	int RegionID = Span->RegionID;

	Span->RegionID = NewRegionID;
	Span->DistanceToRegionCore = 0;
	SpanStack.Add(Span);
	SpanBorderDistance.Add(0);

	while (SpanStack.Num() != 0)
	{
		int ElementToRemove = 0;

		UOpenSpan* CurrentSpan = SpanStack.Last();
		ElementToRemove = SpanStack.Num() - 1;
		SpanStack.RemoveAt(ElementToRemove);

		int Distance = SpanBorderDistance.Last();
		ElementToRemove = SpanBorderDistance.Num() - 1;
		SpanBorderDistance.RemoveAt(ElementToRemove);

		for (int Iter = 0; Iter < 4; Iter++)
		{
			UOpenSpan* NeighborSpan = Span->GetAxisNeighbor(Iter);
			if (!NeighborSpan || NeighborSpan->RegionID != RegionID)
			{
				continue;
			}

			int NeighborDistance = Distance;
			if (Iter == BorderDirection)
			{
				if (Distance == 0)
				{
					continue;
			...
			else if (Iter == AntiBorderDirection)
			{
				NeighborDistance++;
			}
			...
			SpanStack.Add(NeighborSpan);
			SpanBorderDistance.Add(NeighborDistance);
		}
	}
}

void AOpenHeightfield::ResetSpanFlags()
{
	for (auto& Span : Spans)
	{
		UOpenSpan* CurrentSpan = Span.Value;
...
bool AOpenHeightfield::ProcessNullRegion(UOpenSpan* StartSpan, int StartDirection)
{
	int BorderRegionID = StartSpan->RegionID;

	UOpenSpan* Span = StartSpan;
	UOpenSpan* NeighborSpan = nullptr;
	...
	bool HasSingleConnection = false;
...
bool AOpenHeightfield::ProcessOuterCorner(UOpenSpan* Span, int BorderDirection)
{
	bool HasMultiRegions = false;

	int UpdatedDirection = IncreaseNeighborDirection(BorderDirection, 3);

	UOpenSpan* BackOneSpan = Span->GetAxisNeighbor(UpdatedDirection);
	UOpenSpan* BackTwoSpan = BackOneSpan->GetAxisNeighbor(BorderDirection);
	UOpenSpan* TempSpan = nullptr;

	if (BackOneSpan->RegionID != Span->RegionID && BackTwoSpan->RegionID == Span->RegionID)
	{
		HasMultiRegions = true;

		TempSpan = BackOneSpan->GetAxisNeighbor(UpdatedDirection);
		int BackTwoConnections = 0;
		if (TempSpan && TempSpan->RegionID == BackOneSpan->RegionID)
		{
			BackTwoConnections++;
			TempSpan = TempSpan->GetAxisNeighbor(BorderDirection);
			if (TempSpan && TempSpan->RegionID == BackOneSpan->RegionID)
			{
				BackTwoConnections++;
			}
		...
		int SpanConnections = 0;
		UpdatedDirection = IncreaseNeighborDirection(BorderDirection, 2);
		TempSpan = BackOneSpan->GetAxisNeighbor(UpdatedDirection);
		if (TempSpan && TempSpan->RegionID == BackOneSpan->RegionID)
		{
			SpanConnections++;
			TempSpan = TempSpan->GetAxisNeighbor(UpdatedDirection);
			if (TempSpan && TempSpan->RegionID == BackOneSpan->RegionID)
			{
				BackTwoConnections++;
			}
		...
		if (SpanConnections > BackTwoConnections)
		{
			Span->RegionID = BackOneSpan->RegionID;
		}
		else
		{
			BackTwoSpan->RegionID = BackOneSpan->RegionID;
		}
	}
	else if (BackOneSpan->RegionID == Span->RegionID && BackTwoSpan->RegionID == Span->RegionID)
	{
		UpdatedDirection = IncreaseNeighborDirection(BorderDirection, 2);
		int TempDirection = IncreaseNeighborDirection(BorderDirection, 1);
		int SelectedRegion = SelectedRegionID(BackTwoSpan, TempDirection, UpdatedDirection);

		if (SelectedRegion == BackTwoSpan->RegionID)
		{
			UpdatedDirection = IncreaseNeighborDirection(BorderDirection, 3);
			SelectedRegion = SelectedRegionID(Span, BorderDirection, UpdatedDirection);

			if (SelectedRegion != Span->RegionID)
			{
				Span->RegionID = SelectedRegion;
				HasMultiRegions = true;
			}
		}
		else
		{
			BackTwoSpan->RegionID = SelectedRegion;
			HasMultiRegions = true;
		}
	}
	else
...
int AOpenHeightfield::SelectedRegionID(UOpenSpan* Span, int BorderDirection, int CornerDirection)
{
	TArray<int> NeighborRegionsID;

	Span->GetNeighborRegionIDs(NeighborRegionsID);
	int UpdatedDirection = IncreaseNeighborDirection(BorderDirection, 2);
	int RegionID = NeighborRegionsID[UpdatedDirection];

	if (RegionID == Span->RegionID || RegionID == NULL_REGION)
	{
		return Span->RegionID;
	}

	int PotentialRegion = RegionID;
	int UpdateCornerDirection = IncreaseNeighborDirection(CornerDirection, 2);
	RegionID = NeighborRegionsID[CornerDirection];
	...
	{
		if (NeighborRegionsID[Iter] == Span->RegionID)
		{
			CurrentCount++;
		}
		else if(NeighborRegionsID[Iter] == PotentialRegion)
		{
			PotentialCount++;
		}
	}
